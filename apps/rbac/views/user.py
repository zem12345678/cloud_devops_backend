...
from cloud_devops_backend.settings import SECRET_KEY
...
class UserAuthView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user:
            ...
            return OpsResponse({'token': jwt.encode(payload, SECRET_KEY)},status=OK,msg='登录成功')
        else:
            return OpsResponse('用户名或密码错误!', status=BAD)
...
class UserInfoView(APIView):
    ...
    def get(self, request):
        if request.user.id is not None:
            ...
            return OpsResponse(data, status=OK)
        else:
            return OpsResponse('请登录后访问!', status=UNAUTHORIZED)
...
class UserBuildMenuView(APIView):
    ...
    def get(self, request):
        if request.user.id is not None:
            ...
            return OpsResponse(menu_data, status=OK)
        else:
            return OpsResponse('请登录后访问!',status=FORBIDDEN)
...
class UserViewSet(CacheResponseMixin,ModelViewSet):
    ...
    ordering_fields = ('id',)
    authentication_classes = (JSONWebTokenAuthentication,TokenAuthentication, BasicAuthentication, SessionAuthentication)
    ...
    def set_password(self, request, pk=None):
        ...
        if 'admin' in perms or 'user_all' in perms or request.user.is_superuser:
            ...
            if new_password1 == new_password2:
                ...
                user.save()
                return OpsResponse('密码修改成功!')
            else:
                return OpsResponse('新密码两次输入不一致!', status=status.HTTP_400_BAD_REQUEST)
        else:
            ...
            if check_password(old_password, user.password):
                ...
                if new_password1 == new_password2:
                    user.set_password(new_password2)
                    user.save()
                    return OpsResponse('密码修改成功!',status=OK)
                else:
                    return OpsResponse('新密码两次输入不一致!', status=status.HTTP_400_BAD_REQUEST)
            else:
                return OpsResponse('旧密码错误!', status=status.HTTP_400_BAD_REQUEST)

class UserListView(ListCacheResponseMixin,ListAPIView):
    queryset = UserProfile.objects.all()
